...
public class HW4{
	...
	public static void main(String[] args)
	{
		...
		//The neural network data structure configured using the given variables/data
		...
		//Training the NN to learn its weights
		...
		int correct = 0;
		//for each Instance in the test set, output is calculated through NN
		for(int i = 0; i < testSet.size(); i++)
		{
			...
			//Looks through an instance's class values to find correct classification
			for (int j=0; j < testSet.get(i).classValues.size(); j++) {
				if (testSet.get(i).classValues.get(j) > 0.5)
					actual_idx = j;
			}

			//if output calcualted through NN is equal to classValue found above, classification is correct
	...
	private static ArrayList<Instance> getData(String file)
	{
		...
		try{
			...
			while (in.ready()) { 
				...
				//## designates number of attributes (256 in train1.txt)
				else if (prefix.equals("##")) {
					attributeCount=Integer.parseInt(line.substring(2));
				} 

				//** designates number of possible outputs (3 in train1.txt)
				else if (prefix.equals("**")) {
					outputCount=Integer.parseInt(line.substring(2));
				}

				//adding each new instance (line) to the arraylist of Instances, "Data"
				else {
